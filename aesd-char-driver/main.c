/**
 * @file aesdchar.c
 * @brief Functions and data related to the AESD char driver implementation
 *
 * Based on the implementation of the "scull" device driver, found in
 * Linux Device Drivers example code.
 *
 * @author Dan Walkes
 * @date 2019-10-22
 * @copyright Copyright (c) 2019
 *
 */

#include <linux/module.h>
#include <linux/init.h>
#include <linux/printk.h>
#include <linux/types.h>
#include <linux/cdev.h>
#include <linux/fs.h> 	    // file_operations
#include <linux/slab.h>    
#include "aesdchar.h"

int aesd_major =   0; // use dynamic major
int aesd_minor =   0;

MODULE_AUTHOR("Harsh Beriwal"); /** TODO: fill in your name **/
MODULE_LICENSE("Dual BSD/GPL");

struct aesd_dev aesd_device;

int aesd_open(struct inode *inode, struct file *filp)
{
    struct aesd_dev *a_dev = NULL; 
    PDEBUG("open");
    //finding the aesd_dev data structure linked with the inode and assigning it to private_data
    a_dev = container_of(inode ->i_cdev, struct aesd_dev,cdev);
    filp->private_data = a_dev; 
    return 0;
}

int aesd_release(struct inode *inode, struct file *filp)
{
    PDEBUG("release");
    //struct aesd_dev *a_dev;
    filp ->private_data = NULL;
    //a_dev = container_of(inode ->i_cdev, struct aesd_dev,cdev);
    //(void)a_dev;
    return 0;
}

ssize_t aesd_read(struct file *filp, char __user *buf, size_t count,
                loff_t *f_pos)
{
    ssize_t retval = 0;
    size_t offset = 0;
    struct aesd_buffer_entry *element = NULL;
    struct aesd_dev *a_dev = filp->private_data;

    PDEBUG("read %zu bytes with offset %lld\n", count, *f_pos);

    if (mutex_lock_interruptible(&a_dev->lock) != 0) {
        PDEBUG("failed to acquire mutex\n");
	mutex_unlock(&a_dev -> lock); 
    	return retval;
    }

    element = aesd_circular_buffer_find_entry_offset_for_fpos(&a_dev->circularBuffer, *f_pos, &offset);
    if (element != NULL) {
        retval = copy_to_user(buf, (element->buffptr + offset), (element->size - offset));
        retval = (element->size - offset) - retval;
        *f_pos += retval;
    }

    PDEBUG("aesd_read returns %ld\n", retval);

    mutex_unlock(&a_dev->lock);

    return retval;
}


ssize_t aesd_write(struct file *filp, const char __user *buf, size_t count,
                loff_t *f_pos)
{
    ssize_t retval = -ENOMEM;
    const char *lbuffptr = NULL;
    struct aesd_dev *a_dev = filp->private_data;

    PDEBUG("write %zu bytes with offset %lld\n", count, *f_pos);

    if (mutex_lock_interruptible(&a_dev->lock) != 0) {
        PDEBUG("failed to acquire mutex\n");
    	return retval;
    }

    if (a_dev->element.size == 0) {
        a_dev->element.buffptr = (char *) kzalloc(count, GFP_KERNEL);
    } else {
        a_dev->element.buffptr = (char *) krealloc(a_dev->element.buffptr, \
                                a_dev->element.size + count, GFP_KERNEL);
    }

    if (a_dev->element.buffptr == NULL) {
        PDEBUG("failed to allocate memory\n");
        retval = -ENOMEM;
    } else {
        retval = copy_from_user((void *) a_dev->element.buffptr + a_dev->element.size, buf, count);

        retval = count - retval;
        a_dev->element.size += retval;
        if (a_dev->element.buffptr[(a_dev->element.size - 1)] == '\n') {
            lbuffptr = aesd_circular_buffer_add_entry(&a_dev->circularBuffer, &a_dev->element);
            if (lbuffptr != NULL)
                kfree(lbuffptr);
            a_dev->element.buffptr = NULL;
            a_dev->element.size = 0;
        }
    }

    mutex_unlock(&a_dev->lock);

    return retval;
}


struct file_operations aesd_fops = {
    .owner =    THIS_MODULE,
    .read =     aesd_read,
    .write =    aesd_write,
    .open =     aesd_open,
    .release =  aesd_release,
};

static int aesd_setup_cdev(struct aesd_dev *a_dev)
{
    int err, devno = MKDEV(aesd_major, aesd_minor);

    cdev_init(&a_dev->cdev, &aesd_fops);
    a_dev->cdev.owner = THIS_MODULE;
    a_dev->cdev.ops = &aesd_fops;
    err = cdev_add (&a_dev->cdev, devno, 1);
    if (err) {
        printk(KERN_ERR "Error %d adding aesd cdev", err);
    }
    return err;
}



int aesd_init_module(void)
{
    dev_t a_dev = 0;
    int result;
    result = alloc_chrdev_region(&a_dev, aesd_minor, 1,
            "aesdchar");
    aesd_major = MAJOR(a_dev);
    if (result < 0) {
        printk(KERN_WARNING "Can't get major %d\n", aesd_major);
        return result;
    }
    memset(&aesd_device,0,sizeof(struct aesd_dev));

    /**
     * TODO: initialize the AESD specific portion of the device
     */

    mutex_init(&aesd_device.lock);
    //aesd_device.element = kmalloc(sizeof(struct aesd_buffer_entry), GFP_KERNEL);
    aesd_circular_buffer_init(&aesd_device.circularBuffer);

    result = aesd_setup_cdev(&aesd_device);

    if( result ) {
        unregister_chrdev_region(a_dev, 1);
    }
    return result;

}

void aesd_cleanup_module(void)
{
    dev_t devno = MKDEV(aesd_major, aesd_minor);
    struct aesd_buffer_entry *element = NULL;
    int count=0;

    AESD_CIRCULAR_BUFFER_FOREACH(element, &aesd_device.circularBuffer, count) 
	{
		if(element->buffptr != NULL)
		{
			kfree(element->buffptr);
			//element->size = 0;
		}
	}

    cdev_del(&aesd_device.cdev);
    //mutex_destroy(&aesd_device.lock);
    unregister_chrdev_region(devno, 1);
}



module_init(aesd_init_module);
module_exit(aesd_cleanup_module);
